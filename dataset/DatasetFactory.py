...
from toolkit.datasets.vot import VOTDataset, VOTLTDataset
...
class LaSOTClsVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        super(LaSOTClsVideo, self).__init__(name, root, video_dir,
...
class LaSOTClsDataset(Dataset):
    ...
    def __init__(self, name, dataset_root, load_img=False):
        super(LaSOTClsDataset, self).__init__(name, dataset_root)
        with open(os.path.join(dataset_root, 'anno.json'), 'r') as f:
        ...
        for video in pbar:
            ...
            video_dir = meta_data[video]['category']
            dataset_root = ''
            attr = 'None'
            # absent = meta_data[video]['full_occ']
            absent = [1] * len(meta_data[video]['img_names'])

            self.videos[video] = LaSOTClsVideo(video,
                                          dataset_root,
                                          video_dir,
...
class DatasetFactory(object):
    ...
    def create_dataset(**kwargs):
        ...
        if 'OTB' in name:
            ...
            dataset = LaSOTClsDataset(**kwargs)
        elif 'UAV' in name:
